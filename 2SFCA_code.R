...
##### Huron county
### physician to population ratio
dat = sf_loc[sf_loc$County=="Huron",]
dat$pro_to_pop_ratio = 0
Total = nrow(dat)
for (i in 1:Total) {
  print(i)
...
### Spatial access index
ct_dat = ct_centroid[ct_centroid$county_geoid== 39077,]
ct_dat$SPAI = 0
Total = nrow(ct_dat)
for (i in 1:Total) {
  print(i)
...
st_write(ct_dat, dsn = paste("D:/Vaccine_Access/Shapefiles/ct_centroid_huron.shp"),
         driver = "ESRI Shapefile")

### assign values to polygon boundaries
Huron = acs_data1[acs_data1$county_geoid == 39077, ]
Huron$SPAI = ct_dat$SPAI
Huron$SPAR = Huron$SPAI/mean(Huron$SPAI)
st_write(Huron, dsn = paste("D:/Vaccine_Access/Shapefiles/ct_huron.shp"),
         driver = "ESRI Shapefile")
...
##### Vinton county
### physician to population ratio
dat = sf_loc[sf_loc$County=="Vinton",]
dat$pro_to_pop_ratio = 0
Total = nrow(dat)
for (i in 1:Total) {
  print(i)
...
### Spatial access index
ct_dat = ct_centroid[ct_centroid$county_geoid== 39163,]
ct_dat$SPAI = 0
Total = nrow(ct_dat)
for (i in 1:Total) {
  print(i)
...
st_write(ct_dat, dsn = paste("D:/Vaccine_Access/Shapefiles/ct_centroid_vinton.shp"),
         driver = "ESRI Shapefile")

### assign values to polygon boundaries
Vinton = acs_data1[acs_data1$county_geoid == 39163, ]
Vinton$SPAI = ct_dat$SPAI
Vinton$SPAR = Vinton$SPAI/mean(Vinton$SPAI)
st_write(Vinton, dsn = paste("D:/Vaccine_Access/Shapefiles/ct_vinton.shp"),
         driver = "ESRI Shapefile")
